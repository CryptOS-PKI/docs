...
- A tag-gated GitHub Pages deploy (publishing only on `vX.Y.Z`) fails at the Deploy step unless the
  ...
  `gh api -X POST repos/CryptOS-PKI/docs/environments/github-pages/deployment-branch-policies -f name='v*' -f type=tag`
- Docusaurus MDX 3: avoid the `## Heading {#custom-id}` explicit-id syntax (it fails to compile);
  rely on the auto-generated slugs.

### This repo

`docs` ships a documentation site to GitHub Pages, not a package to a registry.
A "release" here is a tagged Pages deploy. It stays on `0.x.y` until the whole
CryptOS system lands -- do not cut `1.0.0` yet.
